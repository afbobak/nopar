...
function iteratePackages(all, fn) {
  for (var i = all.length - 1; i >= 0; i--) {
    var name = all[i];
    if (fs.statSync(path.join(registryPath, name)).isDirectory()) {
      fn(name, exports.getPackage(name));
    }
  }
}
...
exports.query = function (query) {
  ...
  iteratePackages(all, function (name, pkg) {
...
exports.queryLocal = function (query) {
  var all = fs.readdirSync(registryPath);
  var pkgs = {};

  iteratePackages(all, function (name, pkg) {
    if (pkg["_local"] && (!query || (query && name.indexOf(query) >= 0))) {
      pkgs[name] = pkg;
    }
  });
