...
exports.refreshMeta = function (settings) {
  ...
  for (var i = pkgs.length - 1; i >= 0; i--) {
    ...
    if (fs.statSync(path.join(registryPath, name)).isDirectory()) {
      var pkgMeta = exports.getPackage(name, null, settings);
...
exports.getPackage = function (pkgName, version, settings) {
  ...
  var pkgMeta = pkgMetaCache[pkgName];
  if (pkgMeta) {
    return pkgMeta;
  }
...
exports.iteratePackages = function (settings, fn) {
  var all = fs.readdirSync(registryPath);
  iteratePackages(all, settings, fn);
};
...
exports.getDependents = function (packagename, settings) {
  var deps = {
    _counts : {
      runtime : 0,
      dev     : 0
  ...
  exports.iteratePackages(settings, function (name, pkg) {
