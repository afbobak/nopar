...
/**
 * Return distibution tags
 */
exports.distTags = function () {
  return function (req, res) {
    winston.info("DIST-TAGS " + req.originalUrl, req.body);
    ...
    var pkgMeta = registry.getPackage(pkgName);
    if (!pkgMeta) {
      winston.info("Unknown package: " + pkgName + "@" + version);
      return res.status(404).json({
        "error"  : "not_found",
        "reason" : "document not found"
      });
    ...
    res.status(200).json(pkgMeta["dist-tags"]);
  };
};
