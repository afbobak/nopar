...
var defaults = {
  "logfile"               : process.env.NOPAR_LOGFILE || "",
  "loglevel"              : process.env.NOPAR_LOGLEVEL || "info",
  "limit"                 : "10mb",
...
function getRenderer() {
  return function renderSettings(req, res, message) {
    ...
    if (vars.settings.baseUrl === defaults.baseUrl) {
      vars.settings.baseUrl = null;
    }
...
exports.init = function init(registry) {
  ...
  for (key in metaSettings) {
    if (metaSettings.hasOwnProperty(key)) {
...
exports.middleware = function (registry) {
  if (!registry) {
    throw new Error('Missig required argument "registry"');
  }

  var settingsStore = exports.init(registry);
  registry.refreshMeta(settingsStore);

  return function (req, res, next) {
    req.settingsStore = settingsStore;
    next();
  };
};
...
exports.save = function save() {
  ...
  return function (req, res, message) {
    ...
    settings.set("baseUrl", body.baseUrl ||
      settings.defaults["baseUrl"]);
