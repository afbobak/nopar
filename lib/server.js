...
function renderIndex(req, res, message) {
  ...
  if (req.query.q || req.query.local || req.query.p) {
    page = req.query.p ? "package" : "results";
    if (req.query.p) {
      vars.registry = {};
 ...
 */
app.route("/-/settings")
  .get(settings.render(settingsStore))
  .post(settings.save(registry, settingsStore));

/**
 * Packages Actions
 */
app.get("/-/packages/local", function (req, res) {
  // TODO: render a list of local packages
});
app.get("/-/packages/leafs", function (req, res) {
  // TODO: render a list of packages that have no dependents
});
app.get("/-/packages/outdated", function (req, res) {
  // TODO: render a list of packages with outdated dependencies
});
...
app.get("/-/package/:packagename/dependencies", function (req, res) {
});
app.get("/-/package/:packagename/dependencies/dev", function (req, res) {
});
app.get("/-/package/:packagename/dependents", function (req, res) {
});
app.get("/-/package/:packagename/dependents/dev", function (req, res) {
});

/*
 * Web Actions
 */
app.get("/-/package/:packagename/delete", pkg.unpublish(app,
  function (req, res) {
    renderIndex(req, res,
      "Package <strong>" + req.params.packagename +
      "</strong> successfully deleted.");
  }));
app.get("/-/package/:packagename/refresh", pkg.refresh(app,
  function (req, res) {
    renderIndex(req, res,
      "Package <strong>" + req.params.packagename +
      "</strong> successfully refreshed.");
  }));
 ...
 * Attachment methods
 */
app.route("/:packagename/-/:attachment/-rev?/:revision?")
  .get(attachment.download(app))
  .put(attachment.attach(app))
  ["delete"](attachment.detach(app));
