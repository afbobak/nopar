...
exports.route = function (router) {
  ...
  router.get('/package/:scope(@[^\/]+)?/:name', pkg.render);
  router.get('/package/:scope(@[^\/]+)?/:name/dist-tags', pkg.distTags());
  router.get('/package/:scope(@[^\/]+)?/:name/refresh', pkg.refresh());
  router.get('/package/:scope(@[^\/]+)?/:name/delete', pkg.unpublish());
};
